...
<body>
    ...
    <div>
        ...
        <form action="{{route('plate.update', $plate->id)}}" method="POST" class="justify-content-center" style="margin: 56px;width: 440px;height:auto;padding: 29px;" enctype="multipart/form-data">
            {{ csrf_field() }}
            <input type="hidden" name="_method" value="PATCH"></input>
            <h3 style="margin-bottom: 0px">Modificar Plato Especial</h3>
            ...
            <div class="form-group">
                ...
                <input class="form-control" type="text" name="name" placeholder="Nombre" value="{{$plate->name}}">
                @error('name')
                    <div class="aler alert-danger">
                        El campo Nombre es obligatorio
                    </div>
                @enderror
                <label style="margin-top: 10px;"><strong> Descripcion </strong></label>
                <input class="form-control" type="text" name="description" placeholder="Descripcion" value="{{$plate->description}}">
                @error('description')
                    <div class="aler alert-danger">
                ...
                <input class="form-control" type="number" onkeydown="javascript: return event.keyCode === 8 || event.keyCode === 46 ? true : !isNaN(Number(event.key))" name="precio" placeholder="Precio" value="{{$plate->precio}}">
                @error('precio')
                <div class="aler alert-danger">
                ...
                @enderror
                <div id="imagPrev">
                <img style="border: ridge; width: 250px; height: auto;" src="{{asset($plate->photo)}}">
                </div>
            </div>
           
                <div class="form-group"><button class="btn btn-primary btn-block" type="submit" style="background: rgb(230,208,13);height: 50px;width: 152px;margin: 14px;padding: -4px;font-size: 16px;">ActualizarPlato</button>
                </div>
        </form>
